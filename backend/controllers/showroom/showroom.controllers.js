...
const addShowroom = async (req, res) => {
   try {
      ...
      console.error('Error while adding showroom:', error);
...
const getShowroom = async (req, res) => {
   try {
      ...
      console.log(error);
      return res.status(500).json({ success: false, msg: "An error occurred while fetching showroom details. Please try again later" });
   }
}
...
const deleteShowroom = async (req, res) => {
   try {
      ...
      await Inventory.deleteMany({ showroomId });
      await Showroom.findByIdAndDelete(showroomId);

      return res.status(200).json({ success: true, msg: "Showroom and its inventory deleted successfully" });
   } catch (error) {
      console.log(error);

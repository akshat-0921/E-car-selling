...
// --- STYLING: Replaced react-icons with lucide-react for consistency ---
import { SlidersHorizontal } from "lucide-react";

// --- LOGIC: Asset imports are preserved ---
...
// --- LOGIC: Data options are preserved ---
...
// --- STYLING: Custom styles object for react-select to handle light/dark modes ---
const customSelectStyles = (isDarkMode) => ({
   control: (provided) => ({
      ...
      minHeight: '42px',
      '&:hover': { borderColor: isDarkMode ? '#4B5563' : '#9CA3AF' }, // gray-600 : gray-400
      boxShadow: 'none',
   }),
   menu: (provided) => ({
      ...provided,
...
const Filter = ({ onFilterChange }) => {
   // --- LOGIC: State and handlers are entirely preserved ---
   ...
   // --- STYLING: Added state to detect system theme for react-select styling ---
   ...
   // --- UX Improvement: Added reset functionality ---
   const handleReset = () => {
   ...
   return (
      ...
      >
         ...
         {/* --- STYLING: Restructured price range section --- */}
         ...
         {/* --- STYLING: Each filter group has a consistent structure and uses themed react-select --- */}
         ...
         <style jsx global>{`
                .range-slider::-webkit-slider-thumb { @apply appearance-none h-4 w-4 bg-blue-600 rounded-full cursor-pointer; }
                .range-slider::-moz-range-thumb { @apply appearance-none h-4 w-4 bg-blue-600 rounded-full cursor-pointer border-none; }
                .range-slider::-webkit-slider-runnable-track { @apply w-full h-2 bg-slate-200 dark:bg-slate-700 rounded-lg; }
                .range-slider::-moz-range-track { @apply w-full h-2 bg-slate-200 dark:bg-slate-700 rounded-lg; }
            `}</style>
      </form>
   );
};
